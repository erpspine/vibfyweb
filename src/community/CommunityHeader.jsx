...
export default function CommunityHeader() {
  ...
  const host = session?.portalAccesses?.some(access => access.portal === 'host')
  ...
  return <header className="vf-nav community-nav" onKeyDown={e => { if (e.key === 'Escape') close() }}>
    ...
    <nav id="community-navigation" aria-label="Community navigation" className={open ? 'is-open' : ''}>
      ...
      {session?.token && <CommunityLink className="community-host-link" to={host ? '/host' : '/become-host'} onClick={close}><Store size={15} />{host ? 'Host dashboard' : 'Become a host'}<ArrowUpRight size={14} /></CommunityLink>}
      {session?.token ? <div className="community-account"><button className="community-account-trigger" aria-expanded={accountOpen} aria-controls="community-account-links" onClick={() => setAccountOpen(!accountOpen)}><UserRound size={18} /><span>{session.user?.name?.split(' ')[0] || 'My account'}</span><ChevronDown size={14} /></button>{accountOpen && <div className="community-account-menu" id="community-account-links"><small>ONE ACCOUNT. YOUR WHOLE VIBFY.</small><CommunityLink to="/profile" onClick={close}><UserRound />My profile</CommunityLink><CommunityLink to="/bookings" onClick={close}><Ticket />My bookings</CommunityLink><CommunityLink to="/saved" onClick={close}><Bookmark />Saved events</CommunityLink><CommunityLink to={host ? '/host' : '/become-host'} onClick={close}><Store />{host ? 'Switch to Host' : 'Become a host'}</CommunityLink>{session.portalAccesses?.some(a => a.portal === 'manager') && <CommunityLink to="/manager" onClick={close}>Manager dashboard</CommunityLink>}<button onClick={logout} disabled={busy}><LogOut />{busy ? 'Signing out...' : 'Log out'}</button></div>}</div> : <CommunityLink className="vf-button" to="/login" onClick={close}>Join / Log in <ArrowUpRight size={16} /></CommunityLink>}
    </nav>
